...
const withPaintCanvas = (config) => {
    ...
    config = (0, config_plugins_1.withDangerousMod)(config, [
        ...
        async (config) => {
            ...
            const mainAppPath = path.join(config.modRequest.platformProjectRoot, 'app', 'src', 'main', 'java', 'com', config.modRequest.projectName || 'colorplayexpo', 'MainApplication.java');
            if (fs.existsSync(mainAppPath)) {
                ...
                if (!content.includes('new PaintCanvasPackage()')) {
                    content = content.replace(/packages.add\(new ModuleRegistryAdapter/, 'packages.add(new PaintCanvasPackage());\n        packages.add(new ModuleRegistryAdapter');
                }
